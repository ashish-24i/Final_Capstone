import "./video.css"
import { Link } from "react-router-dom";
import { useState, useEffect } from "react";

import { useParams } from 'react-router-dom'


function Video() {

    const [message, setMessage] = useState("");

    const [data, setData] = useState(null);

    const [data1, setData1] = useState([]);


    const [videoUrl, setVideoUrl] = useState(null);

    const { id } = useParams();


    async function fetchData(url) {
        try {
            const res = await fetch(url);
            const data = await res.json();

            setData(data?.video)

            setVideoUrl(data?.video?.videoLink)



        } catch (error) {
            console.error("Error fetching data:", error);
        }
    }


    async function fetchData1(url) {
        try {
            const res = await fetch(url);
            const data = await res.json();


            setData1(data.comments)

        } catch (error) {
            console.error("Error fetching data:", error);
        }
    }


    useEffect(() => {

        fetchData(`http://localhost:3000/api/getVideoById/${id}`);

        fetchData1(`http://localhost:3000/commentApi/comment/${id}`)

    }, [])





    console.log(id)
    return (
        <div className="video absolute top-0 right-0 left-0">
            <div className="videoPostSection">

                <div className="video_youtube">

                    {videoUrl ? (
                        <iframe
                            src={videoUrl}
                            width="850"
                            height="400"
                            className="rounded-3xl"
                            allow="autoplay; fullscreen"
                            frameBorder="0"
                        ></iframe>
                    ) : (
                        <p className="text-white">Loading video...</p>
                    )}

                </div>

                <div className="video_youtube_About">
                    <div className="video_youtube_title"> {data?.title}</div>
                    <div className="youtube_video_profile_block">
                        <div className="youtubeVideoProfileBlockLeft">
                            <Link to={`/user/${data?.user?._id}`} className="youtube_video_profile_block_left_img">
                                <img className="youtube_video_profileBlock_left_image" src={data?.user?.profilePic} alt="alternate" />

                                <div className="youtubeVideo_subsView">
                                    <div className="youtubepostprofilename ">{data?.user?.userName}</div>
                                    {/*<div className="youtubepostprofilesubs ">{"2024-07-09"}</div>*/}

                                </div>

                                <div className="subscribebtnyoutube relative left-32 bottom-8 w-20">Subscribe</div>




                            </Link>
                        </div>

                        <div className="youtubeVideoLikeBlock">

                            <div className="youtube_video_likeBlock_likers    flex flex-row gap-4">
                                <i class="fa-solid fa-thumbs-up  text-white"></i>
                                <div className="youtube_video_likeblock_numoflike">{data?.like}</div>


                                <div className="youtubeVideoDivider w-0 h-[20px] border border-blue-800">

                                </div>

                                <div className="youtube_video_likeBlock_liker">
                                    <i class="fa-solid fa-thumbs-down text-white"></i>
                                </div>
                            </div>






                        </div>

                    </div>

                    <div className="youtube_video_About">
                        <div>2024-09-30</div>
                        <div>{data?.description}</div>
                    </div>
                </div>


                <div className="youtube_comment_section">
                    <div className="youtubecommentsectiontitle">
                        {data1?.length} comments
                    </div>

                    <div className="youtubeselfcomment">
                        <img className="video_youtubeselfcommentprofile" src="data:image/jpeg;base64," />
                        <div className="addAcomment">
                            <input type="text" value={message} onChange={(e) => { setMessage(e.target.value) }} className="addACommentInput" placeholder="Add acomment" />

                            <div className="cancelSubmitComment">
                                <div className="cancelcomment">Cancel</div>
                                <div className="cancelcomment">Comment</div>
                            </div>
                        </div>
                    </div>

                    <div className="youtubeOthersComment">


                        {

                            data1?.map((item, index) => {

                                return (
                                    <div className="youtubeselfcommnt">

                                        <img className="video_youtubeselfcommentprofile" src={item?.user?.profilePic} alt="alternate" />

                                        <div className="othercommentsection">
                                            <div className="othercommentsectionhadder">
                                                <div className="channelName_comment">{item?.user?.channelName}</div>
                                                <div className="commenttimingothers">2025-09-30</div>
                                            </div>

                                            <div className="othercommentsectioncomment">
                                                {item?.message}
                                            </div>
                                        </div>
                                    </div>
                                )
                            })
                        }




                    </div>
                </div>

            </div>



            <div className="videoSuggestion">


                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>

                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>

                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>

                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>

                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>

                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>

                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>

                <div className="videoSuggestionBlock">
                    <div className="video_suggestion_thumbnail">

                        <img className="video_suggestion_thumbnail_img" src="https://i.ytimg.com/vi/CakDDkF-Qv4/hqdefault.jpg?sqp=-oaymwExCOADEI4CSFryq4qpAyMIARUAAIhCGAHwAQH4Af4JgALQBYoCDAgAEAEYZSBkKE8wDw==&rs=AOn4CLDJ-V_5ecO8BLJW8PKAhNjFwR3aWA" />


                    </div>

                    <div className="videoSuggestion_about">
                        <div className="video_suggestion_about_title">
                            T20 2025 Asia CUp Final India Vs Srilanka LAst 5 Over #cricket #India
                        </div>
                        <div className="video_suggestion_about_profile"> Cricket 320</div>
                        <div className="video_suggestion_about_profile"> 136K views . 1 day ago</div>

                    </div>
                </div>
            </div>
        </div>
    )

}


export default Video;
